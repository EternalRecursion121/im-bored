...
<article class="bg-[#18181b] rounded-lg border border-zinc-800 overflow-hidden hover:border-zinc-700 transition-colors">
	{#if item.imageUrl}
	...
	<div class="p-4">
		<div class="flex items-start gap-3 mb-2">
			<div class="w-8 h-8 bg-zinc-800 rounded-md flex items-center justify-center flex-shrink-0">
				{#if typeIcon() === 'article'}
			...
			<div class="flex-1 min-w-0">
				<h3 class="font-medium text-zinc-100 text-sm line-clamp-2">
					{#if item.url}
						<a
							href={item.url}
							target="_blank"
		...
		{#if item.description}
			<p class="text-zinc-500 text-sm line-clamp-3 mb-3">
				{item.description}
			</p>
		{/if}

		{#if item.llmEnrichments?.summary}
			<div class="bg-purple-950/30 rounded-md p-3 mb-3 border border-purple-900/50">
				<div class="text-xs text-purple-400 font-medium mb-1">AI Summary</div>
				<p class="text-sm text-purple-200">{item.llmEnrichments.summary}</p>
			</div>
		{/if}
		...
		{#if item.llmEnrichments?.tags && item.llmEnrichments.tags.length > 0}
			<div class="flex flex-wrap gap-1 mb-3">
				{#each item.llmEnrichments.tags as tag}
					<span class="px-2 py-0.5 bg-zinc-800 text-zinc-400 text-xs rounded">
						{tag}
					</span>
				{/each}
			</div>
		...
		<div class="flex items-center justify-between pt-3 border-t border-zinc-800">
			<div class="flex items-center gap-2 text-xs text-zinc-500">
				{#if item.author}
					<span class="font-medium text-zinc-400">{item.author}</span>
					<span>·</span>
				{/if}
